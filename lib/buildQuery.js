  ...
  if (filters.where && Array.isArray(filters.where)) {
    ...
    filters.where = filters.where
      ...
      .map(({ field, operator, value }) => {
        ...
        var { type } = _.get(assocModel, ['attributes', attribute], {});

        //catch for createdAt & updatedAt attributes, which don't show up in the assocModel
        if (attribute == 'createdAt' || attribute == 'updatedAt') { type = 'timestamp' }
